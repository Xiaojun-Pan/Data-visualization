        ...
        series: [
          // 状态0，1使用散点图呈现
          {
            ...
            data: chartData.categoryData.filter(function(param) {
              return param.value[6] < '2'
            }), // 这里是api数据接受的的地方,使用筛选器只显示状态0和1
            symbol: 'rectangular', // 图形 'circle', 'rect', 'roundRect', 'triangle', 'diamond', 'pin', 'arrow'
            symbolSize: [100, 60], // 标记的大小，可以设置成诸如 10 这样单一的数字，也可以用数组分开表示宽和高，例如 [20, 10] 表示标记宽为20，高为10。
            ...
            label: {
              normal: {
                ...
                formatter: function(param) {
                  ...
                  if (param.value[2] === 'th') {
                    if (param.value[3] !== 'NA') {
                      status = status + param.value[3] + ' ℃\n'
                    }
                    if (param.value[4] !== 'NA') {
                      status = status + param.value[4] + ' %rh\n'
                    }
                  }

                  // 落尘节点
                  if (param.value[2] === 'dust') {
                    if (param.value[5] !== 'NA') {
                      status = status + param.value[5] + ' EA/ft³\n'
                    }
                  }
                  // 异常状态提示
                  if (param.value[6] === '0') {
                    status = status + '节点离线'
                  } else if (param.value[6] === '2') {
                    status = status + '电量不足'
                  } else if (param.value[6] === '3') {
                    status = status + '环境异常'
                  }
